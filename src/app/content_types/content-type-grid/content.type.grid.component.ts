...
import { ContentTypeGridActionComponent } from './content-type-grid-action/content.type.grid.action.component';

@Component({
  selector: 'app-content-type-grid',
  moduleId: module.id,
  templateUrl: 'content.type.grid.component.html',
  styleUrls: ['content.type.grid.component.css'],
})
export class ContentTypeGridComponent implements OnInit, OnDestroy {
  ...
  gridOptions: Partial<GridOptions> = {
    columnDefs: [
      ...
      },
      {
        ...
        filter: false,
        sortable: false,
        cellRenderer: 'contentTypeActionRenderer',
        minWidth: 230,
        maxWidth: 230,
      },
    ],
    components: {
      contentTypeActionRenderer: ContentTypeGridActionComponent,
    },
    pagination: true,
  ...
  onDeleteRows(selectedRows: any[]) {
    let selectedRowsId = selectedRows.map((val) => val.contentTypeId);
    this.deleteSub = this.storageService
      .onDeleteModel('contentType', selectedRowsId)
      .subscribe((data) => {
  ...
  contentTypeGetGridView(page: number) {
    this.subscription = this.storageService
      .getGridView('contentType', {
        sorts: this.gridSort,
  ...
  onPaginationChanged(page: PageEvent) {
    ...
    this.contentTypeGetGridView(page.pageIndex + 1);
  }
